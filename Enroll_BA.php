...
$stmt = $conn->prepare("SELECT BA FROM users WHERE email = ?");
if (!$stmt) {
...
if ($baStatus === "yes") {
    ...
    <body>
      <div class='message-box'>
        <h2>Already Registered for BA Course.</h2>
    ...
    $updateStmt = $conn->prepare("UPDATE users SET BA = 'yes' WHERE email = ?");
    if (!$updateStmt) {
    ...
    if ($updateStmt->execute()) {
        ...
        header("Location: success_BA.html");
        exit();
    } else {
