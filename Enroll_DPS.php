...
$stmt = $conn->prepare("SELECT DPS FROM users WHERE email = ?");
if (!$stmt) {
...
if ($baStatus === "yes") {
    ...
    <body>
      <div class='message-box'>
        <h2>Already Registered for DPS Course.</h2>
    ...
    $updateStmt = $conn->prepare("UPDATE users SET DPS = 'yes' WHERE email = ?");
    if (!$updateStmt) {
    ...
    if ($updateStmt->execute()) {
        ...
        header("Location: success_DPS.html");
        exit();
    } else {
